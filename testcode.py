...
def check_commands_from_drive():
    global stop_recording  # Ensure it's a global variable to modify it
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if command_content.lower() == "stop":
                    ...
                    stop_recording = True  # Stop the audio recording
...
def record_audio():
    ...
    audio_path = os.path.join(LOG_DIR, f"audio_recording.wav")
    ...
    try:
        ...
        with sd.InputStream(samplerate=fs, channels=2) as stream:
            while True:
                audio_data, overflowed = stream.read(chunk_duration * fs)
                if overflowed:
                    logging.warning("Audio buffer overflow!")
                frames.append(audio_data)

                # Check if the stop flag is set, then stop the recording
                if stop_recording:
                    logging.info("Stop command received. Stopping audio recording.")
                    break

        # Save audio data after stopping the recording
        write(audio_path, fs, np.concatenate(frames, axis=0))
